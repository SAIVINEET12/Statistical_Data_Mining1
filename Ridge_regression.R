...
cv_ridge <- cv.glmnet(hw_train_desc, hw_train_target, alpha = 0)
best_lambda <- cv_ridge$lambda.min
plot(cv_ridge)
best_lambda
log(best_lambda)
best_ridge <- glmnet(hw_train_desc, hw_train_target, alpha = 0, lambda = best_lambda)
coef(best_ridge)

# R2 value for Ridge
# For Training set
y_train <- predict(best_ridge, s = best_lambda, newx = hw_train_desc)
...
# For Testing set
y_test <- predict(best_ridge, s= best_lambda, newx = hw_test_desc)
...
plot(hw_test_target,y_test, xlab= 'actual value', ylab='predicted value',main='Ridge Regression')
abline(0,1)
par(mfrow = c(1,1))
